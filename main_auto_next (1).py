...
TASKS = {
    ...
    "choice": "💬 Choose the Answer", "question": "✍️ Short Answer",
    "gap": "✏️ Fill in the Gap",
    "mixed": "🎲 Mixed Tasks",
}
...
- Use these task types: true_false, choice, guess, complete, question and gap.
- If task type is mixed, include no more than ONE written task (question or gap). All other tasks must use buttons.
...
- For question, use an empty options list and require only a 1-4 word written answer.
- For gap, use an empty options list, put exactly one ___ in the sentence and require exactly one word as the answer.
...
Use types: true_false, choice, guess, complete, question and gap. If mixed, include no more than ONE written task (question or gap).
For every choice, guess and complete task include exactly 3 short options, with one option exactly matching answer.
For true_false use an empty options list. For question use an empty options list and require a 1-4 word answer. For gap use an empty options list, exactly one ___ and a one-word answer. Keep everything short.
...
async def callbacks(call: CallbackQuery):
    ...
    if data == "welcome:about":
        ...
        await call.message.answer(f"👀 <b>Answer:</b> {player.tasks[player.task_index]['answer']}")
        player.task_index += 1
        await send_task(call.message, player)
    elif data == "task:next":
        player.task_index += 1
...
async def handle_answer(message: Message, player: Player, answer: str, user_id: int):
    ...
    if result.get("correct"):
        player.score += 1
        await message.answer(f"✅ {result.get('feedback', 'Great job!')} ⭐")
        player.task_index += 1
        await send_task(message, player)
    else:
        await message.answer(
